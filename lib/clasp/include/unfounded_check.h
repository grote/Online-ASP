...
public:
	...
	struct UfsAtomNode : public UfsNode {
		 ...
		 */
		UfsBodyNode*	watch()			const	{ return (UfsBodyNode*)(source_&~1u); }
		...
		void					markSourceInvalid() { source_ |= 1; }
		void					resurrectSource()		{ source_ &= ~1u; }
...
// -------------------------------------------------------------------------------------------	
	...
	bool						invSign_;			// invert sign of reason lits?
};
}
#endif
