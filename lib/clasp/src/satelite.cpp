...
bool SatElite::preprocess() {
	if (options.verbose) options.verbose(0, Options::pre_start);
	struct Scope { SatElite* self; Options::Action action; ~Scope() { 
		self->cleanUp();
		if (self->options.verbose) self->options.verbose(0, action);
	}} sc = {this, Options::pre_done};
	...
	while (!queue_.empty() || !elimHeap_.empty()) {
		++iteration;
		if (options.verbose)				{ options.verbose(iteration, Options::iter_start); }
		...
		while (!elimHeap_.empty())	{
			...
			if (++eliminated == 1000 && timeOut != std::numeric_limits<std::time_t>::max()) {
				eliminated	= 0;
				if (time(0) > timeOut) { 
					sc.action = Options::pre_stopped; 
					elimHeap_.clear(); 
				}
			}
		}
		if (options.verbose) options.verbose(iteration, Options::iter_done);
