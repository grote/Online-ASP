...
\paragraph{-g}
This parameter invokes the new output format for intermediate languages ASPils \cite{atleastthisreferenceishouldgive}. Currently only the ``Conglomeration'' normal form is supported.
...
Furthermore the bitwise operators and(\&), or(?), xor(\textasciicircum) and invert($\sim$) are supported.
...
\begin{verbatim}
 ...
 c(4).
\end{verbatim}
would result in the grounded program\footnote{gringo would simplify the grounded rule to fact, since all literals in the body are facts}
\begin{verbatim}
 a(0) :- b(1,2), c(4).
 a(1) :- b(1,2), c(4).
 a(2) :- b(1,2), c(4).
...
\section{$\Pi$-Restrictiveness (or something else)}
...
\section{Benchmarks}
TODO: run on what machine, give reference to encodings.
