...
Incremental programs must be splitted into 3 parts. The base part, for the information that hold in all steps. The incremental part, where the incremental rules must be set, and the volatile part. In this part the queries to the program are given.
To seperate the part, three statement exists. Everything beyond such a statement is considered to belong to the specific part.
To start the base part
\begin{verbatim}
 #base.
\end{verbatim}
must be written. Now all rules of the base part can follow.
For the incremental part an additional variable has to be passed to the statement.
\begin{verbatim}
 #incremental(X).
\end{verbatim}
Now all incremental rules follow. The incremental variable used for grounding is $X$.
