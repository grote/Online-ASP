...
The first version is gringo2.0, it acts as a grounder and supports several output formats and even a human readable format for debugging purposes. This version can ground logic programs and output them to files or pipe them to the solver.
...
The third package is called iclingo, which stands for ``incremental clingo''. In this version the grounder gringo2.0 can be used with clasp incrementally. Detailed information about incremental solving can be found here \cite{gekaosscth08a} 
In this paper, at first, the general restrictions to the input language are stated. Then a brief description of how to use gringo2.0, clingo and iclingo is given.
...
With this option you can enable or disable the bindersplitting used by gringo2.0
%TODO	Default: yes
\begin{itemize}
 \item yes : Enable bindersplitting
 \item no  : Disable bindersplitting
\end{itemize}
...
The default command line is ``\texttt{./gringo --lparse}'' to read from standard input and for smodels output.
...
It uses the grounder gringo2.0 and the solver clasp, and therefore all options if gringo2.0, clingo and clasp are valid.
...
At least \texttt{num} incremental steps are solved, wether an answer set has been found or not.
...
With this option enabled, iclingo solves only the base program and ignores the incremental and query part of the program.
This option can be helpful when debugging.
\paragraph{\texttt{--istop=<arg>}}
The stop condition during incremental solving be set to ``\texttt{SAT}'' or ``\texttt{UNSAT}'' to stop the search process either if a solution was found (\texttt{SAT} and default) or if no solution was found (\texttt{UNSAT}).
...
All rules that hold in all timesteps that are independent of the incremental constant are put here.
...
\begin{verbatim}
 on(X,Y,k) :- move(X,Y,k), block(X), location(Y).
\end{verbatim}
...
We first define a little helper predicate that states that a block is not on a certain position.
\begin{verbatim}
 not_on(X,Y,k) :- on(X,Z,k), Y != Z, block(X), location(Y;Z).
\end{verbatim}
Now block \texttt{X} is ``\texttt{not\_on}'' a position, where it is not ``\texttt{on}''.
With this helping predicate we can define that a block is on the same position in step \texttt{k}, if it was not ``\texttt{not\_on}'' that position in the step before.
\begin{verbatim}
 on(X,Y,k) :- on(X,Y,k-1), block(X), location(Y), not not_on(X,Y,k).
\end{verbatim}
This trick is often used to describe the state of something that has not changed by an action.
\newline
...
To restrict the moves, an integrity constraint is used.
\begin{verbatim}
 :- move(X,Y,k), on(Z,X,k-1), block(X;Z), location(Y).
\end{verbatim}
So it is not allowed to move block \texttt{X} in step k, if there is another block on it in the state before.
Also you can not move a block to a position where another block is on, so only one block on top of another is allowed.
\begin{verbatim}
 :- move(X,Y,k), on(Z,Y,k-1), block(X;Y;Z).
\end{verbatim}
...
%  7 loc(t).
%  8 loc(X) :- bl(X).
%% und die Positionen t=table, 1 ... n
%  9 
%% Block 1 ist auf Block 2, 2 auf 3 ... und n auf dem Tisch
% 11
% 12 #cumulative k.
% 13
% 14 1{mv(X,Y,k) : bl(X) : loc(Y) : X != Y}1.
%% Wähle jeden Zug einen move von Block X nach Y aus
% 15 on(X,Y,k) :- mv(X,Y,k), bl(X), loc(Y).
%% Ein Block X ist auf der Position Y (im Schritt k)falls block X nach Y bewegt wird(in Schritt k).
% 16 on(X,Y,k) :- on(X,Y,k-1), bl(X), loc(Y), not non(X,Y,k).
% 17 non(X,Y,k) :- on(X,Z,k), Y != Z, bl(X), loc(Y;Z).
%% Ein Bock X ist auf Position Y (in Schritt k) wenn er nicht, nicht dort lag.
%% non(X,Y) gilt im Schritt k, falls X NICHT auf Y liegt.
% 18 :- mv(X,Y,k), on(Z,X,k-1), bl(X;Z), loc(Y).
%% move von X nach Y (im Schritt k) ist falsch, falls im vorherigen Schritt ein Block auf X lag. ???
% 19 :- mv(X,Y,k), on(Z,Y,k-1), bl(X;Y;Z).
%% move von X nach Y (im Schritt k) ist falsch, falls im vorherigen Schritt ein Block auf Y lag
% 20
% 21 #volatile k.
% 22
% 23 :- not on(X+1,X,k), bl(X), X < n.
%% solange keine Antwort, bis n auf n-1 ... 2 auf 1 liegt.
% 24
% 25 #base.
...
\subsubsection{Meta-Syntax for Incremental Programs}
Incremental programs must be splitted into 3 parts.
The base part, for the information that hold in all steps ans is independent of the incremental constant.
...
And finally the volatile part. In this part the queries to the program are given, that must hold in the last state.
To seperate the parts, three statement exists. Everything beyond such a statement is considered to belong to the specific part.
...
\begin{verbatim}
 a(X) :- X=Y, Y=X.
\end{verbatim}
Furthermore the assignment operator can be used with aggregates, for example
\begin{verbatim}
